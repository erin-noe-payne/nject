 ...
 *
 * This is copy / paste from angular.
 *
...
Tree.prototype.register = function (key, fn, opts) {
  ...
  if (this.isRegistered(key)) {
    msg =
      'Naming conflict encountered on ' + identifier + '\n' +
      'Overwriting registered dependency with new definition.'
    emitLog(self, LOG_LEVEL.WARN, msg)
    this.clear(key)
  }
  ...
  if(!constant) {
    ...
    dependencies = this.findDependencies(fn);
  }
...
Tree.prototype._getToResolve = function(key, list) {
  var self =this,
    registered = this._registry[key],
    list = list || []

  if(!registered) {
    throw new Error('Detected unregistered dependency: '+key+'.')
  }

  list.unshift(key)

  var dependencies = registered.dependencies
  _.each(dependencies, function(dependency) {
    if(!_.contains(list, dependency)) {
      self._getToResolve(dependency, list)
    }
  })
...
Tree.prototype._findCircularDependencies = function(toResolve) {
  var self = this,
    first = _.first(toResolve)
  ...
  function walk(key, path) {
    var config = self._registry[key]
    // This possibility should not truly be possible since by definition
    // a module with 0 dependencies is resolvable.
    ...
    if (firstInstance >= 0) {
      ...
      path.push(config.identifier);
      var nextMod = _.find(config.dependencies, function (dep) {
        return _.contains(toResolve, dep)
      });
...
Tree.prototype._getResolutionOrder = function(toResolve) {
  var self          = this,
    prevLength      = -1,
    resolutionOrder = [],
    nextLength      = resolutionOrder.length,
    unresolved      = toResolve,
    i = 0;

  // Loops at least once with the default values since 0 > -1; where
  // prevLength defaults to -1, and resolutionOrder defaults to 0.
  while ((nextLength > prevLength) && (i < 4)) {
    i++
    // Stall out if we are not continuing to resolve dependencies.
    pushToResolveOrder(unresolved, resolutionOrder);
    prevLength = nextLength;
  ...
  function pushToResolveOrder(unresolved, resOrder) {
    _.each(_.clone(unresolved), function (key) {
      var config = self._registry[key]
      if (_.difference(config.dependencies, resOrder).length == 0) {
        resOrder.push(key);
        _.remove(unresolved, function(n){return key==n})
      }
    });
  };
...
Tree.prototype._resolve = function (key) {
  var self = this,
...
Tree.prototype.resolve = function (key) {
  ...
  if(!key) {
    throw new Error('Provide a dependency key to resolve.')
  }

  // 1: Get list of dependencies that will need to be resolved
  // 2: Identify any unregistered dependencies and print trace
  toResolve = this._getToResolve(key)
  console.log(toResolve)

  // 3: Build resolution order
  resolutionOrder = this._getResolutionOrder(toResolve)
  console.log(resolutionOrder)

  // 4: Identify any circular dependencies and print trace
  unresolved = _.difference(toResolve, resolutionOrder)
  if(unresolved.length > 0) {
    throw new Error(this._findCircularDependencies(unresolved))
  }

  //4: resolve dependencies if not already resolved
